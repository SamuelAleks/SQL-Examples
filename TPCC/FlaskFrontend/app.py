...
def calculate():
    ...
    try:
        ...
        cursor.execute("INSERT INTO `ORDER` (O_ID, O_D_ID, O_W_ID, O_C_ID, O_ENTRY_D, O_CARRIER_ID, O_OL_CNT, O_ALL_LOCAL) VALUES (%s, %s, %s, %s, %s,%s, %s, %s)",
                       (d_next_o_id, district, warehouse, customer, order_date, 1, ol_cnt, o_all_local,))

        # cursor.execute("INSERT INTO NEW_ORDER (NO_W_ID, NO_D_ID, NO_O_ID) VALUES (%s, %s, %s)",
        #               (warehouse, district, d_next_o_id,))
        ...
        for i in range(ol_cnt):
            ...
            cursor.execute("INSERT INTO ORDER_LINE (OL_O_ID, OL_D_ID, OL_W_ID, OL_NUMBER, OL_I_ID, OL_SUPPLY_W_ID, OL_DELIVERY_D, OL_QUANTITY, OL_AMOUNT, OL_DIST_INFO) VALUES (%s, %s, %s, %s, %s, %s, NULL, %s, %s, %s)",
                           (d_next_o_id, district, warehouse, i + 1, 1, 1, ol_quantity_i, 1, s_dist_xx))
    ...
    for row in rows:
        ...
        # table_rows.append([row[1], row[0], item_names[i], row[2], stock_list[i], brand_list[i], item_prices[i], int(stock_list[i]) * int(item_prices[i])])
